...
data1 <- read_xlsx("C:/Users/UNALMED/Desktop/One Health/Tesis doctora Olga/Bases de datos/5. Resultados Citometría CyC_22032024.xlsx", sheet = 3)
names(data)
names(data1)
```
...
```{r}
vari_s1 <- c( "IL_6_23", "IL_10_25" )
variCi_s1 <- c("Number_of_Relative_Events_T_Helper_cells_CD4_cells_5", "Number_of_Relative_Events_T_Cytotoxic_Cells_CD8_cells_7" )

vari_s2 <- c( "IL_6_49", "IL_10_51" )
variCi_s2 <- c("Number_of_Relative_Events_T_Helper_cells_CD4_cells_12", "Number_of_Relative_Events_T_Cytotoxic_Cells_CD8_cells_14")

vari_s3 <- c( "IL_6_69", "IL_10_71" )
variCi_s3 <- c("Number_of_Relative_Events_T_Helper_cells_CD4_cells_26", "Number_of_Relative_Events_T_Cytotoxic_Cells_CD8_cells_28")
```
...
#Llamar la función con las variables de Citometría de la base de datos
data1 <- convertir_a_numerico(data1, variCi_s1)
data1 <- convertir_a_numerico(data1, variCi_s2)
data1 <- convertir_a_numerico(data1, variCi_s3)
```


```{r}
# Crear un subconjunto de los datos que excluye los valores de ID_SAMPLE "GOB-HSPL-0045", "GOB-HSPL-0102" para la imputación de las muestras de las visitas 2 y 3
data_subset <- data[!data$ID_SAMPLE %in% c("GOB-HSPL-0045", "GOB-HSPL-0102"), ]
data1_subset <- data1[!data1$ID_SAMPLE %in% c("GOB-HSPL-0045", "GOB-HSPL-0102"), ]
```
...
```{r}
imputed_data1_sample1 <- mice(data1[variCi_s1], m=5, maxit=50, method='pmm', seed=500 )
```


```{r, include=FALSE}
#Imputar las variables de Il de la muestra 2 con la libreria mice
imputed_data_sample2 <- mice(data_subset[vari_s2], m=5, maxit=50, method='pmm', seed=500)
```

```{r}
imputed_data1_sample2 <- mice(data1_subset[variCi_s2], m=5, maxit=50, method='pmm', seed=500 )
```

```{r, include=FALSE}
#Imputar las variables de Il de la muestra 3 con la libreria mice
imputed_data_sample3 <- mice(data_subset[vari_s3], m=5, maxit=50, method='pmm', seed=500)
```

```{r}
imputed_data1_sample3 <- mice(data1_subset[variCi_s3], m=5, maxit=50, method='pmm', seed=500 )
```

```{r}
# Completar cada conjunto de datos imputado de interleucinas
...
```{r}
# Completar cada conjunto de datos imputado de Citometría
completed_data1_sample1 <- complete(imputed_data1_sample1, 1)
completed_data1_sample2 <- complete(imputed_data1_sample2, 1)
completed_data1_sample3 <- complete(imputed_data1_sample3, 1)
```
...
summary(completed_data_sample1[,vari_s1])
```
...
```{r}
#visualizar estadisticos descriptivos de las variables Il de la muestra 3 sin imputar 
summary(data1_subset[,variCi_s3])
```

```{r}
#visualizar estadisticos descriptivos de las variables Il de la muestra 3 imputada 
summary(completed_data1_sample3[,variCi_s3])
```

## Grafico de distribución para las variables de interleucina de la muestra 1 imputadas y sin imputar
```{r}
par(mfrow=c(1,2))  # Organizar los gráficos en una matriz de 10 filas y 3 columnas

for (var in vari_s1) {
    # Calcular las densidades
    dens_orig <- density(data[[var]], na.rm=TRUE)
    dens_imputed <- density(completed_data_sample1[[var]])
...
## Grafico de distribución para las variables de citometría de la muestra 1 imputadas y sin imputar
```{r}
par(mfrow=c(1,2))  # Organizar los gráficos en una matriz de 10 filas y 3 columnas

for (var in variCi_s1) {
    # Calcular las densidades
    dens_orig <- density(data1[[var]], na.rm=TRUE)
    dens_imputed <- density(completed_data1_sample1[[var]])
...
## Grafico de distribución para las variables de interleucina de la muestra 2 imputadas y sin imputar
```{r}
par(mfrow=c(1,2))  # Organizar los gráficos en una matriz de 10 filas y 3 columnas

for (var in vari_s2) {
    # Calcular las densidades
    dens_orig <- density(data_subset[[var]], na.rm=TRUE)
    dens_imputed <- density(completed_data_sample2[[var]])
...
for (var in variCi_s2) {
    # Calcular las densidades
    dens_orig <- density(data1_subset[[var]], na.rm=TRUE)
    dens_imputed <- density(completed_data1_sample2[[var]])
...
## Grafico de distribución para las variables de interleucina la muestra 3 imputadas y sin imputar
```{r}
par(mfrow=c(1,2))  # Organizar los gráficos en una matriz de 10 filas y 3 columnas
...
for (var in vari_s3) {
    # Calcular las densidades
    dens_orig <- density(data_subset[[var]], na.rm=TRUE)
    dens_imputed <- density(completed_data_sample3[[var]])
...
## Grafico de distribución para las variables de citometría de la muestra 3 imputadas y sin imputar
```{r}
par(mfrow=c(1,2))  # Organizar los gráficos en una matriz de 10 filas y 3 columnas

for (var in variCi_s3) {
    # Calcular las densidades
    dens_orig <- density(data1_subset[[var]], na.rm=TRUE)
    dens_imputed <- density(completed_data1_sample3[[var]])
